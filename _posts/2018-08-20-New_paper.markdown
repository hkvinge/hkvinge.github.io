...
Note that because we assume that all secants have been normalized prior to project, kappa-values are between 0 and 1. In general, if the "intrinsic" dimension of data is greater than k, then the kappa-value for projection dimension k will be very small (for example, less than .2). 
